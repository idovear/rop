...
public class DefaultSecurityManager implements SecurityManager {
    ...
    public MainError validateSystemParameters(RopRequestContext context) {
        ...
        if (context.getAppKey() == null) {
            return MainErrors.getError(MainErrorType.MISSING_APP_KEY, context.getLocale(), context.getMethod(),
                    context.getVersion(), SystemParameterNames.getAppKey());
        }
        if (!appSecretManager.isValidAppKey(context.getAppKey())) {
            return MainErrors.getError(MainErrorType.INVALID_APP_KEY, context.getLocale(), context.getMethod(),
                    context.getVersion(), context.getAppKey());
        }
        ...
        if (context.getVersion() == null) {
            ...
            if (!ropContext.isValidVersion(context.getMethod(), context.getVersion())) {
                return MainErrors.getError(MainErrorType.UNSUPPORTED_VERSION, context.getLocale(), context.getMethod(),
                        context.getVersion());
            }
        }
        ...
        if (context.getServiceMethodDefinition().isObsoleted()) {
            return MainErrors.getError(MainErrorType.METHOD_OBSOLETED, context.getLocale(), context.getMethod(),
                    context.getVersion());
        }
        ...
        if (!MessageFormat.isValidFormat(context.getFormat())) {
            return MainErrors.getError(MainErrorType.INVALID_FORMAT, context.getLocale(), context.getMethod(),
                    context.getVersion(), context.getFormat());
        }
            ...
            for (String fileFieldName : fileFieldNames) {
                ...
                if (paramValue != null) {
                    if (paramValue.indexOf("@") < 0) {
                        return MainErrors.getError(MainErrorType.UPLOAD_FAIL, rrctx.getLocale(), rrctx.getMethod(),
                                rrctx.getVersion(), "MESSAGE_VALID:not contain '@'.");
                    } else {
                        ...
                        if (fileUploadController.isExceedMaxSize(fileContent.length)) {
                            return MainErrors.getError(MainErrorType.UPLOAD_FAIL, rrctx.getLocale(), rrctx.getMethod(),
                                    rrctx.getVersion(), "EXCEED_MAX_SIZE:" + fileUploadController.getMaxSize() + "k");
                        }
                    }
                }
            }
        ...
        if (httpActions.length > 0) {
            ...
            if (!isValid) {
                mainError = MainErrors.getError(MainErrorType.HTTP_ACTION_NOT_ALLOWED, ropRequestContext.getLocale(),
                        ropRequestContext.getMethod(), ropRequestContext.getVersion(), ropRequestContext.getHttpAction());
            }
        }
        return mainError;
    }
        ...
        if (context.isSignEnable()) {
            if (!context.getServiceMethodDefinition().isIgnoreSign()) {
                if (context.getSign() == null) {
                    return MainErrors.getError(MainErrorType.MISSING_SIGNATURE, context.getLocale(), context.getMethod(),
                            context.getVersion(), SystemParameterNames.getSign());
                } else {
